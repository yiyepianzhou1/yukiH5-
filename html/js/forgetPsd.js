...
Zepto(function ($) {
    ...
    // 发送验证码
    var userName = null;
    $('.sendVerificationCode').on('tap', function () {
        console.log(111);
    ...
    function sendCode(phoneNumber) {
        $.ajax({
            type: 'post',
            url: 'http://192.168.0.178:8084/h5/h5sendsms/sendSMSForForgetPwd',
            dataType: 'json',
            data: {
                phone: phoneNumber
            },
    ...
    // 下一步
    function nextSend(phoneNumber, code) {
        $.ajax({
            type: 'post',
            ...
            success: function (date) {
                ...
                if (date.state == 'success') {
                    ...
                    window.location.href = 'newPsd.html';
                }
                if (date.state == 'error') {
                    popBox(date.errMsg);
                }
